@extends('layout/master')

@section('content')
<div class="container">    
    <article style="float:left;width:75%;">
        <h2><a id="watisglasvezel">News article</a></h2>        
        <p>This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text.</p>
        <h2><a id="watisglasvezel">News article</a></h2>        
        <p>This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text.</p>
        <h2><a id="watisglasvezel">News article</a></h2>        
        <p>This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text.</p>
        <h2><a id="watisglasvezel">News article</a></h2>        
        <p>This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text. This is the article text.</p>
    </article>
    <section class="four">
        <ul style="list-style:none;padding:0;margin:0;">
            <li>
                <p>Latest downloads</p>
            </li>
            <ul style="list-style:none;padding:0;margin:0;">
                <li><a class="green" href="#" onclick="scroll('#watisglasvezel');">Download entry</a></li>
                <li><a class="green" href="#" onclick="scroll('#waaromglasvezel');">Download entry</a></li>
                <li><a class="green" href="#" onclick="scroll('#waaromglas');">Download entry</a></li>
            </ul>            
        </ul>
    </section>
    <div style="clear:both;"></div>
</div> 
@endsection
